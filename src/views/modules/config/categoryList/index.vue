        ...
        <el-col>
          <el-form ref="searchForm" hide-details size="mini" inline :model="params">
            <el-form-item label="类别">
              <el-input v-model.trim="params.typeName" placeholder="请输入" />
            </el-form-item>
...
<script>
import { typeInfoList, delType } from '@/api/config';
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.loading = true;
      typeInfoList(params).then(({ result }) => {
    ...
    del(item) {
      this.$confirm('确认要删除吗?', '删除提示', {
        ...
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => delType(item.id)).then(() => {
        this.$message.success('删除成功');
        this.getList();
      }).catch(() => {});
    },
    download() {
      this.$download('/typeinfo/export', { ...this.params });
    }
  }
};
</script>
